    ...
    public void setNewCoordinates(Pointer pointer, MotionEvent event) {
        if (pointer.getMoving1()) {
            if(pointer.activity.getZoomed() == false) {
                pointer.setPoint1(event.getRawX() - pointer.getRange() / 2, event.getRawY());
                ...
                pointer.setPoint1(nx - pointer.getRange() / 2, ny);
                pointer.notifyObserver();
            }
        }
            ...
            if(pointer.activity.getZoomed() == false) {
                pointer.setPoint2(event.getRawX() - pointer.getRange() / 2, event.getRawY());
                pointer.notifyObserver();
            } else {
                ...
                pointer.setPoint2(nx - pointer.getRange() / 2, ny);
                pointer.notifyObserver();
            }
        }
    }
