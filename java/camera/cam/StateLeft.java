...
public class StateLeft implements PointerState {
    @Override
    public void drawPointer(Canvas canvas, Pointer pointer) {
        ...
        float x1,x2,y1,y2;
        x1 = pointer.getPoint1X() + pointer.getRange() - pointer.RANGE/20;
        y1 = pointer.getPoint1Y() + pointer.getRange()/2;
        x2 = pointer.getPoint2X() + pointer.getRange() - pointer.RANGE/20;
        y2 = pointer.getPoint2Y() + pointer.getRange()/2;
        ...
        if (pointer.activity.getZoomed() && (pointer.getMoving1() || pointer.getMoving2())) {
            ...
            Bitmap b = Calculations.addWhiteBorder(pointer.activity.bitmap, pointer.getRange());

            Bitmap resizedBitmap = Bitmap.createBitmap(b,
                    (int) (pointer.getPoint1X() - pointer.getRange() / 2),
                    (int) (pointer.getPoint1Y() - pointer.getRange() / 2), pointer.getRange(), pointer.getRange(), matrix, true);

            if (pointer.getMoving1()) {
                resizedBitmap = Bitmap.createBitmap(b,
                        (int) (pointer.getPoint1X() + pointer.getRange()),
                        (int) (pointer.getPoint1Y() + pointer.getRange()), pointer.getRange(), pointer.getRange(), matrix, true);
            } else if (pointer.getMoving2()) {
                resizedBitmap = Bitmap.createBitmap(b,
                        (int) (pointer.getPoint2X()+ pointer.getRange()),
    ...
    public void setNewCoordinates(Pointer pointer, MotionEvent event) {
        if (pointer.getMoving1()) {
            if (pointer.activity.getZoomed() == false) {
                pointer.setPoint1(event.getRawX(), event.getRawY() - pointer.getRange() / 2);
                ...
                pointer.setPoint1(nx, ny - pointer.getRange() / 2);
                pointer.notifyObserver();
            }
        }
            ...
            if (pointer.activity.getZoomed() == false) {
                pointer.setPoint2(event.getRawX(), event.getRawY() - pointer.getRange() / 2);
                pointer.notifyObserver();
            } else {
                ...
                pointer.setPoint2(nx, ny - pointer.getRange() / 2);
                pointer.notifyObserver();
            }
        }
    }
