...
public class StateRight implements PointerState {
    @Override
    public void drawPointer(Canvas canvas, Pointer pointer) {
        ...
        float x1, x2, y1, y2;
        x1 = pointer.getPoint1X() + pointer.RANGE / 20;
        y1 = pointer.getPoint1Y() + pointer.getRange() / 2;
        x2 = pointer.getPoint2X() + pointer.RANGE / 20;
    ...
    public void setNewCoordinates(Pointer pointer, MotionEvent event) {
        if (pointer.getMoving1()) {
            if (pointer.activity.getZoomed() == false) {
                pointer.setPoint1(event.getRawX() - pointer.getRange(), event.getRawY() - pointer.getRange() / 2);
                ...
                pointer.setPoint1(nx - pointer.getRange(), ny - pointer.getRange() / 2);
                pointer.notifyObserver();
            }
        } else if (pointer.getMoving2()) {
            if (pointer.activity.getZoomed() == false) {
                pointer.setPoint2(event.getRawX() - pointer.getRange(), event.getRawY() - pointer.getRange() / 2);
                pointer.notifyObserver();
            } else {
                ...
                pointer.setPoint2(nx - pointer.getRange(), ny - pointer.getRange() / 2);
                pointer.notifyObserver();
            }
        }
    }
